      ...
      <v-container>
        <v-row>
          <v-col v-for="pattern in VIBRATION_PATTERN_LIST" :key="pattern">
            <v-btn @click="() => setPattern(pattern)" :active="currentVibrationPattern == pattern"
              :disabled="passed > 0" active-color="primary">{{ pattern }}</v-btn>
          </v-col>
        </v-row>
        <v-row>
          <v-col>
            <v-slider :model-value="currentTotal" :max="MAX_TOTAL" step="60000" @update:model-value="setTotal"
              label="トータル駆動時間" :disabled="passed > 0"></v-slider>
          </v-col>
          <v-col cols="auto">
            {{ totalFormatString }}
          </v-col>
        ...
        <v-row>
          <v-col>
            <v-slider :model-value="currentDelay" :max="MAX_DELAY" step="60000" @update:model-value="setDelay"
              label="振動終了切上" :disabled="passed > 0"></v-slider>
          </v-col>
          <v-col cols="auto">
            {{ delayFormatString }}
          </v-col>
        ...
        <v-row>
          <v-col>
            <v-slider :model-value="currentCutoff" :max="MAX_CUTOFF" step="60000" @update:model-value="setCutoff"
              label="振動開始遅延" :disabled="passed > 0" reverse></v-slider>
          </v-col>
          <v-col cols="auto">
            {{ cutoffFormatString }}
          </v-col>
...
const { setPattern, start, stop, setDelay, setTotal, setCutoff, enableVibrationFeature, canStart, canStop, currentStatus, currentVibrationPattern, currentDelay, currentTotal, currentCutoff, passed } = useVibrationPattern();
