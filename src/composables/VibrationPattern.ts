...
const T1_PERIOD = 2 * 60 * 1000; // 2分間隔
const T2_PERIOD = 30 * 60 * 1000; // 30分間隔
const T3_PERIOD = 180 * 60 * 1000; // 3時間間隔

const PERIOD_MAP: Record<VIBRATION_PATTERN, number> = {
  未選択: 0,
  T1: T1_PERIOD,
  T2: T2_PERIOD,
  T3: T3_PERIOD,
...
export function useVibrationPattern() {
  ...
  function computeVibrationStatus(): VIBRATION_STATUS {
    const period = PERIOD_MAP[currentVibrationPattern.value];
    if (period === 0) {
      return '停止';
    }
    return passed.value % period < VIBRATION_PERIOD ? '振動中' : '停止';
  }
  function process() {
